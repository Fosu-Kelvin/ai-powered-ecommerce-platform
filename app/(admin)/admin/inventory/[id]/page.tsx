...
import { getCategoryAttributeConfig } from "@/lib/constants/category-attributes";
...
function CustomAttributesEditor(handle: DocumentHandle) {
  const { data: customAttributes } = useDocument({
    ...handle,
    path: "customAttributes",
  });
  const editCustomAttributes = useEditDocument({
    ...handle,
    path: "customAttributes",
  });
  ...
  const updateAttribute = (
    ...
    const nextAttributes = attributes.map((attribute, currentIndex) =>
      currentIndex === index ? { ...attribute, [field]: fieldValue } : attribute
    );
    editCustomAttributes(nextAttributes);
  };
  ...
  const removeAttribute = (index: number) => {
    editCustomAttributes(attributes.filter((_, currentIndex) => currentIndex !== index));
  };
    ...
    <div className="space-y-3">
      {attributes.length === 0 && (
        <p className="text-sm text-zinc-500 dark:text-zinc-400">
          No custom attributes yet. Add key details like Brand, Size, or Voltage.
        </p>
      )}

      {attributes.map((attribute, index) => (
        <div
          key={attribute._key ?? `${index}`}
          className="grid gap-2 rounded-lg border border-zinc-200 p-3 sm:grid-cols-[1fr_1fr_auto] dark:border-zinc-700"
        >
          <Input
          ...
          <Input
            ...
            placeholder="Attribute value (e.g., Samsung)"
          />
          <Button
            type="button"
            variant="outline"
...
function ProductDetailContent({ handle }: { handle: DocumentHandle }) {
  const { data: name } = useDocument({ ...handle, path: "name" });
  const { data: categoryData } = useDocumentProjection<ProductCategoryProjection>({
    ...handle,
    projection: `{"categorySlug": category->slug.current}`,
  });
  const categoryConfig = getCategoryAttributeConfig(
    categoryData?.categorySlug ?? null
  );
  const showAttributesCard =
    categoryConfig.supportsMaterial ||
    categoryConfig.supportsColor ||
    categoryConfig.supportsDimensions;
        ...
        <div className="space-y-6 lg:col-span-2">
          ...
          <div className="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900 sm:p-6">
            ...
            <p className="mb-4 text-sm text-zinc-500 dark:text-zinc-400">
              Add category-specific details that do not fit standard fields.
            </p>
            <Suspense fallback={<Skeleton className="h-20" />}>
              <CustomAttributesEditor {...handle} />
            </Suspense>
          </div>
          ...
          <div className="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900 sm:p-6">
            ...
            <div className="space-y-4">
              ...
              {categoryConfig.supportsAssembly && (
                <div className="flex items-center justify-between">
                  <div>
                    <p className="font-medium text-zinc-900 dark:text-zinc-100">
                      Assembly Required
