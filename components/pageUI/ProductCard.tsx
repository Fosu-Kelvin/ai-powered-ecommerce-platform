...
export function ProductCard({ product }: ProductCardProps) {
  ...
  const customAttributes = ((product as Record<string, unknown>).customAttributes ??
    []) as Array<{
    name?: string | null;
    value?: string | null;
  }>;
  const attributePreview = customAttributes
    .filter(
      (attribute) =>
          ...
          <div className="flex flex-wrap gap-2">
            {attributePreview.map((attribute) => (
              <span
                ...
                className="rounded-full bg-zinc-100 px-2.5 py-1 text-[11px] font-medium text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300"
              >
