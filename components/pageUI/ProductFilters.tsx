...
export function ProductFilters({ categories }: ProductFiltersProps) {
  ...
  const filterConfig = getCategoryFilterConfig(currentCategory);
  const showColorFilter = filterConfig.supportsColor;
  const showMaterialFilter = filterConfig.supportsMaterial;
  ...
  const visibleCategories = activeParentSlug
    ? categories.filter(
        (category) =>
          category.slug === activeParentSlug ||
          category.parentCategory?.slug === activeParentSlug,
      )
    : parentCategories;
  const sortedCategories = [...visibleCategories].sort((a, b) => {
    const aIsParent = !a.parentCategory;
    const bIsParent = !b.parentCategory;
    ...
    if (a.parentCategory?.title && b.parentCategory?.title) {
      const parentCompare = a.parentCategory.title.localeCompare(
        b.parentCategory.title,
      );
  ...
  const isColorActive = showColorFilter && !!currentColor;
  const isMaterialActive = showMaterialFilter && !!currentMaterial;
      ...
      <div>
        ...
        <Select
          ...
          onValueChange={(value) => {
            ...
            const nextFilterConfig = getCategoryFilterConfig(nextCategory);

            updateParams({
              category: nextCategory,
              color: nextFilterConfig.supportsColor ? currentColor || null : null,
              material: nextFilterConfig.supportsMaterial
          ...
          <SelectContent>
            ...
            {sortedCategories.map((category) => (
              <SelectItem key={category._id} value={category.slug ?? ""}>
                {activeParentSlug && category.parentCategory
                  ? category.title
