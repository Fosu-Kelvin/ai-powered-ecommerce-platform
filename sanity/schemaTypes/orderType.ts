...
export const orderType = defineType({
  ...
  fields: [
        ...
        defineArrayMember({
          ...
          fields: [
            ...
            defineField({
              ...
              type: "number",
              description: "Price at time of purchase in GHS", // Updated
              validation: (rule) => rule.required(),
            }),
          ],
          preview: {
            ...
            prepare({ title, quantity, price, media }) {
              return {
                title: title ?? "Product",
                subtitle: `Qty: ${quantity} • GH₵${price}`, // Updated symbol
                media,
              };
            },
          },
    ...
    defineField({
      ...
      group: "details",
      readOnly: true,
      description: "Total order amount in GHS", // Updated
    }),
    defineField({
      name: "status",
      type: "string",
    ...
    defineField({
      name: "paystackReference", // Renamed from stripePaymentId
      title: "Paystack Reference", // Renamed
      type: "string",
      group: "payment",
      readOnly: true,
      description: "Paystack transaction reference", // Updated
    }),
    defineField({
      name: "createdAt",
      type: "datetime",
  ...
  preview: {
    ...
    prepare({ orderNumber, email, total, status }) {
      return {
        ...
        subtitle: `${email ?? "No email"} • GH₵${total ?? 0} • ${status ?? "paid"}`, // Updated symbol
      };
    },
  },
  orderings: [
